...
export class RoleTester {
  ...
  public static async checkUpdateRole(user: { token: string }, data: any, expectedStatusCode: number = 200) {
    const response = await request(app).put('/role').set({ Authorization: `Bearer ${user.token}` }).send(data);
    expect(response.statusCode).toBe(expectedStatusCode);
    if (expectedStatusCode === 200) {
      expect(response.body.roleName).toBe(data.roleName);
